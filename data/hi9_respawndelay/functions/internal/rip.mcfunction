...
execute if score @s Hi9_RDly_Time matches ..-1 if score #Lang Hi9_RDly_Var matches 0 run tellraw @s [{"text":"[Hi9_RespawnDelay] リスポーン時間が設定されていないため、即時にリスポーンします。","color":"red"},{"text":" おそらく 個人設定 が正しく設定されていません。","color":"gray"}]
execute if score @s Hi9_RDly_Time matches ..-1 if score #Lang Hi9_RDly_Var matches 1 run tellraw @s [{"text":"[Hi9_RespawnDelay] Because no respawn delay is set, you will respawn immediately.","color":"red"},{"text":" Check Individual Spawn Delay.","color":"gray"}]

execute if score @s Hi9_RDly_Time matches ..-1 run function hi9_respawndelay:internal/respawn